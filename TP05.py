# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 10:55:57 2020
...
class Gift:
    ...
    def __init__(self, kind):
        ...
        if kind == "small":
            ...
            raise ValueError(f"Error {kind} is unknown")#renvoyer une erreur au lieu de none et interrompt le programme
...
#au lieu de return self.max_load - sum(gift.weight for gift in self.gifts)
...
#on manipule le traineau comme une liste
    def __len__(self):
...
# Question 0 : lancer le code et allez sur la page http://localhost:8080/gift
# recuperez le fichier bottle2.py sur le dépôt et mettez la dans votre dossier
...
def process_gift(self):
    ...
    if Sledge.take_gift(gift): # a remplacer par quelque chose, c'est la question 2
#   if Sledge.take_gift(self.gift) == True :
        sledge = Sledge()
        yield f"{gift} added to sledge <br/> sledge is now {sledge.gifts}"
    else:
        # Question 3
        sledge = Sledge.ship()#on vide le traineau
...
# pas de question, renvoie juste un json
...
"""Question 5 : ajouter un statut aux cadeaux est il new, wrapped, waiting
 ready, shipping ou delivered"""


"""Question 6 : modifier le code pour mettre a jour le statut des cadeaux au fur et a mesure"""
